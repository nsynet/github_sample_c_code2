...
#define ADDR_MAX9271            0x40   /* use max9271/max96705 */
...
#undef   USE_G35_CAMERA         /* 使用安森美ON AR0147(sensor)+AP0101(ISP),1280x720@25fps */
#define  USE_G59_CAMERA         /* 使用 OV_OX01F10 摄像头,1280x960@25fps */

#ifdef CONFIG_SENSOR_OV10635
#define ADDR_OV_SENSOR  0x30
#elif defined(USE_G59_CAMERA)
#define ADDR_OV_SENSOR 0x36     //使用 OV_OX01F10 摄像头,I2C地址为 0x6C >> 1
#endif
#define ADDR_AP_SENSOR  0x5D

#ifdef USE_G59_CAMERA
#define OV_SENSOR_X1F_ID 0x58

/* 摄像头内参存储的首地址 */
#define SENSOR_PARAM_BASE_ADDR 0x8000

//内参格式为 2byte(整个内参长度)+N byte(内参数据)+4byte(CRC32校验前面的2+N 字节)
#define SENSOR_PARAM_HEAD_LEN   2  /* 2byte(整个内参长度) */
#define SENSOR_PARAM_CRC_LEN    4  /*4byte(CRC32校验前面的2+N 字节)*/

#define OVChip_SYS_BASE (0x8000)
#define OVChip_SYS_SSRR (0x0000)
#define OVChip_SYS_CRSR (0x0010)
#define OVChip_SYS_WDMR (0x0018)
#define OVChip_SYS_WDCR (0x001C)
#define OVChip_SYS_SLSRR_0 (0x0088)

#define HOSTCMD_SF_ID 0x81
#define HOSTCMD_SF_MODE 0x00
#define HOSTCMD_SF_RD_ADDR 0xE700
#define Chip_Start_SCCB_ADDR 0xB000
#define Chip_HOSTCMD_START_ADDR 0xE400
#define Chip_HOSTCMD_TRIGGER_REGISTER 0x8160
#define Chip_HOSTCMD_CHECK_STATUS 0x8180

#define SF_PAGE_SIZE 0x100
#define Chip_PAGE_SIZE (32)

#define HOSTCMD_STATUS_DONE 0x99
#endif

static unsigned int g_max9286_width = 1280;
#ifdef USE_G35_CAMERA
static unsigned int g_max9286_height = 720;
#else
static unsigned int g_max9286_height = 960;
#endif

#define MAX9286_PERIODIC_CHECK   (1000)

#define MAX9286_REG_0X49               0x49
...
struct sensor_data {
        ...
        struct delayed_work init_camera_work;
                                                                                        ...
                                                                                        unsigned short reg, unsigned char *val)
{
        ...
        for (retry = 0; retry < I2C_BUS_READ_WRITE_COUNT_MAX; retry++) {
                if (i2c_master_send(max9286_data->i2c_client, u8_buf, buf_len) < 0) {
                        DEV_LOGD_ERR(&max9286_data->i2c_client->dev,
                                "read reg error on send: reg=0x%x, retry = %d.", reg, retry);
                        msleep(5);
                        continue;
                }
                if (i2c_master_recv(max9286_data->i2c_client, &u8_val, 1) != 1) {
                        DEV_LOGD_ERR(&max9286_data->i2c_client->dev,
                                "read reg error on recv: reg=0x%x, retry = %d.", reg, retry);
                        msleep(5);
                        continue;
                }
                break;
        ...
        if (retry >= I2C_BUS_READ_WRITE_COUNT_MAX) {
                DEV_LOGD_INFO(&max9286_data->i2c_client->dev,
                        "read reg error: reg=0x%x.", reg);
                return -1;
        }
                                                                                                ...
                                                                                                unsigned short reg, unsigned char val)
{
        ...
        for (retry = 0; retry < I2C_BUS_READ_WRITE_COUNT_MAX; retry++) {
                if (i2c_master_send(max9286_data->i2c_client, u8_buf, buf_len) < 0) {
                        DEV_LOGD_ERR(&max9286_data->i2c_client->dev,
                                "write reg error: reg=0x%x, val=0x%x, retry = %d.", reg, val, retry);
                        msleep(5);
                        continue;
                }
                break;
        ...
        if (retry >= I2C_BUS_READ_WRITE_COUNT_MAX) {
                DEV_LOGD_INFO(&max9286_data->i2c_client->dev,
                        "write reg error: reg=0x%x, val=0x%x.", reg, val);
                return -1;
        }
...
static int ov10635_check_device(struct sensor_data *max9286_data, int index)
{
        ...
        if (reg != 0xA6) {
                DEV_LOGD_ERR(&max9286_data->i2c_client->dev,
                        "OV10635 hasn't been found, reg[0x%x] = 0x%x., index=%d",
                        OV10635_REG_PID, reg, index);
                return -1;
        }
        ov10635_read_reg(max9286_data, index, OV10635_REG_VER, &reg);
        if (reg != 0x35) {
                DEV_LOGD_ERR(&max9286_data->i2c_client->dev,
                        "OV10635 hasn't been found, reg[0x%x] = 0x%x.", OV10635_REG_VER, reg);
                return -1;
        }
        DEV_LOGD_INFO(&max9286_data->i2c_client->dev, "OV10635 index=%d was found.", index);
...
static int ov10635_initialize(struct sensor_data *max9286_data, int index)
{
        ...
        DEV_LOGD_INFO(&max9286_data->i2c_client->dev, "index = %d.", index);
...
static inline int max9271_read_reg(struct sensor_data *max9286_data, int index, u8 reg)
{
        ...
        if (retry >= I2C_BUS_READ_WRITE_COUNT_MAX) {
                DEV_LOGD_INFO(&max9286_data->i2c_client->dev,
                        "read reg error: reg=%2x,err=%d", reg,val);
                return -1;
        }
...
static int max9271_write_reg(struct sensor_data *max9286_data, int index, u8 reg, u8 val)
{
        ...
        if (retry >= I2C_BUS_READ_WRITE_COUNT_MAX) {
                DEV_LOGD_ERR(&max9286_data->i2c_client->dev,
                        "write reg error:reg=%2x,val=%2x,err=%d", reg, val,ret);
                return -1;
        }
...
{
        unsigned char i;
        LOGD_INFO("max9271_dump_registers: index = %d.", index);
        for (i = 0; i < 0x20; i++)
                LOGD_INFO("MAX9271 Reg 0x%02x = 0x%x.\r", i, max9271_read_reg(max9286_data, index, i));
}

static void max9286_dump_registers(struct sensor_data *max9286_data)
{
        unsigned char i;
        LOGD_INFO("Dump MAX9286 registers!");
        for (i = 0; i < 0x72; i++)
                LOGD_INFO("MAX9286 Reg 0x%02x = 0x%x.\r", i, max9286_read_reg(max9286_data, i));
}
#else
static void max9271_dump_registers(struct sensor_data *max9286_data, int index)
{
...
/**
 * camera_pwr_reset - Reset the power for camera
 *
 * @max9286_data: the struct data for max9286
 *
 */
static void camera_pwr_reset(struct sensor_data *max9286_data)
{
        gpio_set_value(max9286_data->pwr_gpio, 0);
        msleep(1);
        gpio_set_value(max9286_data->pwr_gpio, 1);
}
...
static int max9286_hardware_preinit(struct sensor_data *max9286_data)
{
        ...
        DEV_LOGD_INFO(&max9286_data->i2c_client->dev, "In %s()", __func__);
        ...
        max9286_write_reg(max9286_data, 0x0A, 0xFF);

        /*Close MAX9286 Local ACK*/
        max9286_write_reg(max9286_data, 0x34, 0x35);

        //MAX96705 Crossbar setup
        ...
        //Set MAX96705/7 double alignment mode
        max9271_write_reg(max9286_data, 0, 0x67, 0xC4);

        //Check MAX96705/7 HS/VS timing meet MAX9286 requirements

        max9271_write_reg(max9286_data, 0, 0x47, 0x2D);  //0x2d5190 = 0x2d37a8(VSYNC HIGH) +  0x0019e8(VSYNC LOW)
        max9271_write_reg(max9286_data, 0, 0x48, 0x37);
        max9271_write_reg(max9286_data, 0, 0x49, 0xA8);
        max9271_write_reg(max9286_data, 0, 0x4A, 0x00);
        max9271_write_reg(max9286_data, 0, 0x4B, 0x19);
        max9271_write_reg(max9286_data, 0, 0x4C, 0xE8);
        msleep(10);

        //ebable vs generation
        max9271_write_reg(max9286_data, 0, 0x43, 0x25);
        msleep(10);

        //Set MAX96707 works on Coaxial mode
        max9271_write_reg(max9286_data, 0, 0x4D, 0xC0);
        max9271_write_reg(max9286_data, 0, 0x0E, 0x06);
        msleep(5);

        //MAX9286 sync mode setup (25.44fps 74.25MHz PCLK)
        max9286_write_reg(max9286_data, 0x06, 0x90);  //25fps,74.25mhz/25= 2970000 = 0x2d5190
        max9286_write_reg(max9286_data, 0x07, 0x51);
        max9286_write_reg(max9286_data, 0x08, 0x2D);
        msleep(10);
        
        //Disable MAX9286 overlap window and frame error threshold
        max9286_write_reg(max9286_data, 0x63, 0x00);
        max9286_write_reg(max9286_data, 0x64, 0x00);
        max9286_write_reg(max9286_data, 0x62, 0x1F);
        max9286_write_reg(max9286_data, 0x61, 0xFF);

        //VS INV disable
        max9286_write_reg(max9286_data, 0x0c, 0x91);

        //Setup MAX9286 output data type YUV422-8bit
        max9286_write_reg(max9286_data, 0x12, 0xF3);
        
        //MAX9286 Frame Internally Generate Mode
        max9286_write_reg(max9286_data, 0x01, 0x00);
        msleep(10);

        //Enable AUTOMASK
        //max9286_write_reg(max9286_data, 0x69, 0xB0);

        //Disable MAX96705 spread spectrum
        max9271_write_reg(max9286_data, 0, 0x02, 0x1F);
        msleep(10);

        //Change MAX96705 configure link to video link
        max9271_write_reg(max9286_data, 0, 0x04, 0x83);
        msleep(5);
        ...
        max9286_data->last_camera_status = reg&0x0f;
        
        LOGD_INFO("max9286_mipi: 0x49 reg = 0x%02x.", reg);
        LOGD_INFO("max9286_mipi: sensor number = %d.", max9286_data->sensor_num);

        if (max9286_data->sensor_num == 0) {
                LOGD_ERR("no camera connected.");
                return -1;
        }
        return 0;
...
static int max9286_hardware_init(struct sensor_data *max9286_data)
{
        ...
        /* Enable all links, If the camera that is not connected is disabled,
        the 0x49 register will not be updated automatically when the camera is plugged in again */
        max9286_write_reg(max9286_data, 0x00, 0xEF);
        /* if no camera connected in link x,Mask it by max9286(0x69) */
        max9286_write_reg(max9286_data, 0x69, ~(0x40 | max9286_data->sensor_is_there));
        ...
        for (i = 1; i <= MAX9271_MAX_SENSOR_NUM; i++) {
                ...
                msleep(1);
                LOGD_INFO("max9286_mipi: initialized sensor  = 0x%02x.", i);
...
#ifdef USE_G59_CAMERA   /*  摄像头内参代码移植自保隆公司相关代码 */

/*    4个摄像头的i2c地址都是0x36，所以默认摄像头的i2c不会被搜索到，读取G59摄像头内参前，需要操作GPIO开启指定的摄像头i2c，入参i=1~4 指哪个摄像头*/
static int OVChipSetLCCEN_High(struct sensor_data *p_max9286_data , unsigned char i) 
{
    if((i >MAX9271_MAX_SENSOR_NUM )||(0 == i)||(NULL == p_max9286_data))
    {
        return -1;
    }
    
    max9271_write_reg(p_max9286_data, i, 0x0e, 0x2a);
    max9271_write_reg(p_max9286_data, i, 0x0f, 0x3e);
    msleep(10);
    return 0;
}

/*    4个摄像头的i2c地址都是0x36，所以默认摄像头的i2c不会被搜索到，读取G59摄像头内参后，需要恢复GPIO关闭摄像头的i2c，入参i=1~4 指哪个摄像头*/
static int OVChipSetLCCEN_Low(struct sensor_data *p_max9286_data , unsigned char i) 
{
    if((i >MAX9271_MAX_SENSOR_NUM )||(0 == i)||(NULL == p_max9286_data))
    {
        return -1;
    ...
    max9271_write_reg(p_max9286_data, i, 0x0f, 0x16);
    msleep(10);
    return 0;
}


static inline int OVChipReadRegister(struct sensor_data *p_max9286_data , int index, unsigned short reg, unsigned char *pdata, unsigned int len, bool addr_16_8)
{
    unsigned char master_data[2] = {0};
    unsigned char slave_data[64];
    unsigned int buf_len = 2;
    int i= 0, retry, timeout = 20;

    if((pdata==NULL)||(NULL == p_max9286_data))
        return -1;

    if(addr_16_8 != 0) {
        // 16bits RegAddr
        master_data[0] = reg >> 8;
        master_data[1] = reg & 0xff;
    }
    else {
        // 8bits RegAddr
        master_data[0] = reg & 0xff;
        buf_len = 1;
    }

    p_max9286_data->i2c_client->addr = ADDR_OV_SENSOR;

    for (retry = 0; retry < timeout; retry ++) {
        if (2 != i2c_master_send(p_max9286_data->i2c_client, master_data, buf_len)) {
            dev_dbg(&p_max9286_data->i2c_client->dev,
                "%s:write reg error: reg=%x, retry = %d.\n", __func__, reg, retry);
            msleep(10);
            continue;
        }

        if (len != i2c_master_recv(p_max9286_data->i2c_client, slave_data, len)) {
            dev_dbg(&p_max9286_data->i2c_client->dev,
    ...
    if (retry >= timeout ) {
        dev_err(&p_max9286_data->i2c_client->dev,
            "%s:read reg error: reg=%x,len=%d.\n", __func__, reg, len);
        return -2;
    }

    //pr_info("The %dth OVChipReadRegister %d data: \n", index, len); //一次只能读32个字节

    for(i=0; i<len; i++){
        pdata[i] = slave_data[i]; //by ADDR_OV_SENSOR huo qu x1F_ID fuzhigei pdata
    }
...
static inline int OVChipWriteRegister(struct sensor_data *p_max9286_data , int index, unsigned short reg, unsigned char *pdata, unsigned int len, bool addr_16_8)
{
    unsigned char master_data[64] = {0};
    unsigned int buf_len = 2;
    int i = 0, retry, timeout = 20;

    if(NULL == p_max9286_data)
    {
        return -2;
    }
    if (addr_16_8 != 0)
...
#if 0
        /* 开始写传入的数据 */
        pr_info("The %dth OVChipWriteRegister %d data: \n", index, buf_len);
        for(i=0; i<buf_len; i++) {
            pr_info("0x%02x \n", master_data[i]);
        }
#endif
    ...
    for (retry = 0; retry < timeout; retry++)
    {
        if (i2c_master_send(p_max9286_data->i2c_client, master_data, buf_len) < 0)
        {
            dev_dbg(&p_max9286_data->i2c_client->dev,
                    "%s:write reg error: reg=0x%x, val=0x%x, retry = %d.\n", __func__, reg, pdata[0], retry);
            
            LOGD_INFO("df_camera_log: %s:%d", __func__,__LINE__);
            msleep(10);
    ...
    if (retry >= timeout)
    {
        dev_info(&p_max9286_data->i2c_client->dev,
                 "%s:write reg error: reg=0x%x, val=0x%x.\n", __func__, reg, pdata[0]);
...
static inline int OVChipWriteAndReadRegister(struct sensor_data *p_max9286_data , int index, unsigned short reg, unsigned char *pdata, unsigned int len, bool addr_16_8)
{
    int ret = 0;
    int i;
    unsigned char reg_val[64] = {0};

    ret = OVChipWriteRegister(p_max9286_data,index, reg, pdata, len, 1);
    if (ret)
    {
        pr_info("==== write err ==== 0x%04x (%2d) === 0x%x  \n", reg, len, pdata[0]);
        return -1;
    }


    OVChipReadRegister(p_max9286_data ,index, reg, reg_val, len, 1);

    for (i = 0; i < len; i++)
    {

        if (reg_val[i] != pdata[i])
        {
            pr_info("########Register:0x%02x: Write(0x%02x):Read(0x%02x)########\n", reg + i, pdata[i], reg_val[i]);
        }
    }
    return 0;
}

static inline int OVChipIspConfigureEnable(struct sensor_data *p_max9286_data , int index)
{
    int ret = 0;
    ...
    addr = 0xa10a;
    value = 0x00;
    ret = OVChipWriteRegister(p_max9286_data,index, addr, &value, 1, 1);
...
static inline int OVChipIdCheck(struct sensor_data *p_max9286_data , int index, unsigned char *pdata)
{
    int ret = 0;
    unsigned short addr = 0x300a;   /*G59摄像头的id值是0x58 */

    ret = OVChipReadRegister(p_max9286_data,index, addr, pdata, 1, 1);
...
void OVChipReboot(struct sensor_data *p_max9286_data , int index)
{
    //step1: clear watchdog status
    unsigned short addr = OVChip_SYS_BASE + OVChip_SYS_CRSR;
    unsigned char value = 0x00; //clear all software status
    unsigned char ndata[4] = {0, 0, 0, 0};
    int value32 = (BIT31 + BIT30 + 0x400);
    OVChipWriteRegister(p_max9286_data ,index, addr, &value, 1, 1);
    //end of step1
    //step2: enable watchdog to reboot FW from SPI flash
    //clear watchdog count register
    addr = OVChip_SYS_BASE + OVChip_SYS_WDCR;
    OVChipWriteRegister(p_max9286_data ,index, addr, ndata, 4, 1);
    //set watchdog time period register and enable watchdog, wait for reboot.
    ndata[3] = (value32 >> 24) & 0xff;
    ndata[2] = (value32 >> 16) & 0xff;
    ndata[1] = (value32 >> 8) & 0xff;
    ndata[0] = value32 & 0xff;
    addr = OVChip_SYS_BASE + OVChip_SYS_WDMR;
    OVChipWriteRegister(p_max9286_data ,index, addr, ndata, 4, 1);
    //end of step2
}

int OVChipHostCmd_Flash(struct sensor_data *p_max9286_data , int index, unsigned char *data, unsigned int len)
{
    ...
    //send host command
    nCrcBufOffset = 0;
    for (j = 0; j < len; j += Chip_PAGE_SIZE)
    {
        if ((len - j) >= Chip_PAGE_SIZE)
            data_len = Chip_PAGE_SIZE;
        else
            data_len = len - j;
        ...
        // pr_info("############ nPageaddr= %04x #################### ====\n", nPageAddr);

        OVChipWriteRegister(p_max9286_data ,index, nPageAddr, (data + j), data_len, 1);

    }

    addr = Chip_HOSTCMD_TRIGGER_REGISTER;
    value = 0x01;
    OVChipWriteRegister(p_max9286_data,index, addr, &value, 1, 1);

    nTryTime = 0;
    nValue[0] = 0x00;
    addr = Chip_HOSTCMD_CHECK_STATUS;
    msleep(100);
    OVChipReadRegister(p_max9286_data,index, addr, nValue, 1, 1);
    while (nValue[0] != HOSTCMD_STATUS_DONE)
    {
        ...
        if (nTryTime >= 100)
        {
            ...
            pr_info("############ Write and Read--hostcmd--read Register 8180 Status ==== %02x.\n", nValue[0]);
            return FALSE;
        }
    }
    // pr_info("############ Write and Read--hostcmd--read Register 8180 Status ==== %02x.\n",nValue[0]);
...
void OVChipGetDataFromDevice(struct sensor_data *p_max9286_data , int index, unsigned char *data, unsigned int len)
{
    unsigned int nPageAddr = 0;
    unsigned short j = 0;
    ...
    for (j = 0; j < len; j += Chip_PAGE_SIZE)
    {
        ...
        OVChipReadRegister(p_max9286_data,index, nPageAddr, (data + j), nSegment, 1);
    }
}

int OVChipReadFlash(struct sensor_data *p_max9286_data , int index, unsigned int flash_addr, unsigned char *data, unsigned int len)
{
    unsigned int j = 0;
    ...
    ret = OVChipIspConfigureEnable(p_max9286_data,index); //这个在读一段数据之前必须初始化一次，不然会一直报:0x55
    ...
    for (j = 0; j < len; j += SF_PAGE_SIZE)
    {

        if ((len - j) >= SF_PAGE_SIZE)
        {
            data_len = SF_PAGE_SIZE;
        }
        else
        {
            data_len = len - j;
        ...
        pCmdBuf[3] = 0x06;
        pCmdBuf[4] = 0x12;
        pCmdBuf[5] = ((flash_addr + j) >> 16) & 0xff;
        pCmdBuf[6] = ((flash_addr + j) >> 8) & 0xff;
        pCmdBuf[7] = ((flash_addr + j) >> 0) & 0xff;
        pCmdBuf[8] = (data_len >> 8) & 0xff;
        pCmdBuf[9] = (data_len >> 0) & 0xff;

        OVChipWriteRegister(p_max9286_data,index, 0x8181, &date, 1, 1);
        if (OVChipHostCmd_Flash(p_max9286_data,index, pCmdBuf, 10) == FALSE)
        {
            pr_info("OVChipHostCmd_Flash exec err! Cmd:0x12 READ\n");
            return -2;
        }
        else
        {
            OVChipGetDataFromDevice(p_max9286_data,index, (data + j), data_len);
        }
    }
    return 0;
}
...
static ssize_t max9286_reset_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
        int camera_link_reg = 0;
        int hw_camera_link_reg = 0;
        int link_flag = 0;
        ssize_t count;
        struct sensor_data *data = dev_get_drvdata(dev);
        
        camera_link_reg = max9286_read_reg(data, 0x49);
        hw_camera_link_reg = max9286_read_reg(data, 0x48);
        link_flag = max9286_read_reg(data, 0x69);
        count = sprintf(buf, "reset action = %d, camera link = 0x%x, hw_camera_link_reg = 0x%x,link_flag=0x%x\n", data->is_link_reset, camera_link_reg, hw_camera_link_reg,link_flag);
        return count;
}

static ssize_t max9286_reset_store(struct device *dev,
                                        struct device_attribute *attr,
                                        const char *buf, size_t count)
{
        int rst_9286_value = 0;
        rst_9286_value = simple_strtoul(buf, 0,0);
        struct sensor_data *data = dev_get_drvdata(dev);

        LOGD_ERR("%s:%d", __FUNCTION__,__LINE__);
        return 0;  
        
        if(rst_9286_value == 1)
        {
                data->is_link_reset = true;
                /* The camera now uses external synchronization in manual mode. If the synchronization is
                based on 4 channels,one of the cameras must be resynchronized when disconnected. */
                schedule_delayed_work(&data->init_camera_work, 0);
        } else {
                LOGD_ERR("Please input correct parameter!");
        }
        return count;
}

static DEVICE_ATTR(max9286_reset, 0644, max9286_reset_show, max9286_reset_store);

#ifdef USE_G59_CAMERA
/* 摄像头内参的节点，可以通过类似如下读取摄像头3#的内参： 
    echo 3 > /sys/devices/platform/58226000.i2c/i2c-8/8-006a/camera_parameter 
    busybox hexdump /sys/devices/platform/58226000.i2c/i2c-8/8-006a/camera_parameter 
    store操作用来选定1～4 哪个camera，show操作用来获取该摄像头的内参
*/
extern u32 __pure crc32_le(u32 crc, unsigned char const *p, size_t len);
#define INVALID_PARAMETER_LENGTH          (0xFFFF)
static ssize_t camera_parameter_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int ret = 0;
    char chip_id[2]={0};
    struct sensor_data *max9286_data = dev_get_drvdata(dev);
    char length[SENSOR_PARAM_HEAD_LEN] = {0};/*内参长度 */
    unsigned short content_len = 0;          /*内参长度 */
    char *p_content = NULL;   /*内参内容 */
    char crc_read[SENSOR_PARAM_CRC_LEN] = {0};
    unsigned int crc_read_val = 0;
    unsigned int crc_calculated =0;
        
    LOGD_INFO("df_camera_log:%s :camera=%d ++", __func__,max9286_data->camera_index);
    
    OVChipSetLCCEN_High(max9286_data,max9286_data->camera_index);
    
    /* 检查摄像头的ID是否正确 */
    ret = OVChipIdCheck(max9286_data,max9286_data->camera_index,chip_id);    
    
    if(OV_SENSOR_X1F_ID != chip_id[0])
    {
        LOGD_INFO("df_camera_log:%s: unknown camera id =0x%x,0x%x", __func__,chip_id[0], OV_SENSOR_X1F_ID);
        goto EXIT;
    }

    /* 获取内参的长度*/
    ret = OVChipReadFlash(max9286_data, max9286_data->camera_index ,SENSOR_PARAM_BASE_ADDR ,length,SENSOR_PARAM_HEAD_LEN);
    content_len = (length[0] <<8)|length[1];
    
    if((0 == content_len)||( INVALID_PARAMETER_LENGTH == content_len)||(ret != 0))
    {
        LOGD_INFO("df_camera_log:%s:%d %d =>0x%02x,0x%02x,%d", __FUNCTION__,__LINE__,max9286_data->camera_index,length[0],length[1],ret);
        goto EXIT;
    }

    /* 读取内参*/
    p_content =kmalloc((content_len+1),GFP_KERNEL);
    if(NULL == p_content)
    {
        LOGD_INFO("df_camera_log:%s:%d", __FUNCTION__,__LINE__);
        goto EXIT;
    }
    memset(p_content,0,(content_len+1));
    ret = OVChipReadFlash(max9286_data, max9286_data->camera_index ,SENSOR_PARAM_BASE_ADDR ,p_content,content_len);    
    if(ret != 0)
    {
        LOGD_INFO("df_camera_log:%s:%d,ret=%d", __FUNCTION__,__LINE__,ret);
        goto EXIT;
    }
    
    print_hex_dump(KERN_NOTICE," " ,0,16,1,p_content,content_len,false);                

    /* 校验内参的CRC*/
    ret = OVChipReadFlash(max9286_data,max9286_data->camera_index ,SENSOR_PARAM_BASE_ADDR + content_len ,crc_read,SENSOR_PARAM_CRC_LEN);    
    if(ret != 0)
    {
        LOGD_INFO("df_camera_log:%s:%d,ret=%d", __FUNCTION__,__LINE__,ret);
        goto EXIT;
    ...
    crc_read_val = (crc_read[0]<<24)|(crc_read[1]<<16)|(crc_read[2]<<8)|(crc_read[3]);
    crc_calculated = crc32_le(0xffffffff ,p_content , content_len);        
    
    if(crc_calculated == crc_read_val)
    {
        int i =0;
        
        LOGD_INFO("df_camera_log:%s:%d camera=%d,CRC check ok=0x%08x", __FUNCTION__,__LINE__ ,max9286_data->camera_index  ,crc_calculated);

        memcpy(buf,p_content,(content_len+1));
        ret =  content_len;
    }
    else
    {
        LOGD_INFO("df_camera_log:%s:%d camera=%d, CRC calc=0x%08x,CRC read=0x%08x", __FUNCTION__,__LINE__,max9286_data->camera_index ,crc_calculated,crc_read_val);
        ret = 0;
    }
    
    EXIT:
        OVChipSetLCCEN_Low(max9286_data,max9286_data->camera_index);
        if(p_content)
        {
            kfree(p_content);
        }
...
/* store操作用来选定1～4 哪个camera */
static ssize_t camera_parameter_store(struct device *dev,
                    struct device_attribute *attr,
                    const char *buf, size_t count)
{
    struct sensor_data *max9286_data = dev_get_drvdata(dev);
    int  i = simple_strtoul(buf, 0,0);
    
    if((i >=1)&&(i<= MAX9271_MAX_SENSOR_NUM))
    {
        max9286_data->camera_index = i;    
        LOGD_INFO("df_camera_log: %s:%d,camera_index=%d", __FUNCTION__,__LINE__,max9286_data->camera_index);
        return count;
    }
    else
    {
        max9286_data->camera_index = 0;
        return -1;
    }
}

static DEVICE_ATTR(camera_parameter, 0644, camera_parameter_show, camera_parameter_store);
#endif

static struct attribute *max9286_attrs[] = {
        &dev_attr_max9286_reset.attr,
#ifdef USE_G59_CAMERA
    &dev_attr_camera_parameter.attr,
#endif
        NULL
};

static struct attribute_group max9286_group = {
        .attrs = max9286_attrs,
};
...
static int ov10635_change_mode(struct sensor_data *max9286_data)
{
        ...
        if (mode > ov10635_mode_MAX || mode < ov10635_mode_MIN) {
                LOGD_ERR("Wrong ov10635 mode detected!");
                return -1;
        }
        ...
        if (max9286_data->format.width == 0 ||
            ...
            pModeSetting == NULL || ArySize == 0) {
                LOGD_ERR("Not support mode=%d %s", mode,
...
static int max9286_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *a)
{
        ...
        LOGD_INFO("df_camera_log: %s  a->type =%d, ++", __func__, a->type);
        switch (a->type) {
        ...
        case V4L2_BUF_TYPE_SLICED_VBI_OUTPUT:
                ...
                break;
        default:
                LOGD_ERR("   type is unknown - %d", a->type);
                ret = -EINVAL;
                break;
        }
...
static int max9286_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *a)
{
        ...
        case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
                ...
                if (frame_rate != OV10635_30_FPS && frame_rate != OV10635_15_FPS) {
                        LOGD_ERR(" The camera %d frame rate is not supported!", frame_rate);
                        return -EINVAL;
                }

                if (mode > ov10635_mode_MAX || mode < ov10635_mode_MIN) {
                        LOGD_ERR("The camera mode[%d] is not supported!", mode);
                        return -EINVAL;
                }
                ...
                LOGD_INFO("df_camera_log: %s TYPE_VIDEO_CAPTURE", __func__);
                break;
                                   ...
                                   struct v4l2_subdev_frame_interval_enum *fie)
{
        ...
        if (fie->width == 0 || fie->height == 0 ||
            fie->code == 0) {
                LOGD_ERR("Please assign pixel format, width and height.");
                return -EINVAL;
        }
                           ...
                           struct v4l2_subdev_format *fmt)
{
        ...
        LOGD_INFO("df_camera_log: %s,which=%d,pad=%d  ++", __func__,fmt->which,fmt->pad);
        ...
        LOGD_INFO("df_camera_log: %s , mf->code =%d, mf->width=%d, mf->height=%d, mf->colorspace=%d, mf->field=%d, mf->reserved[0]=%d", __func__, mf->code, mf->width, mf->height, mf->colorspace, mf->field, mf->reserved[0]);
        return 0;
}
        ...
        if ((mf->width == ov10635_mode_info_data[frame_rate][mode].width) &&
                ...
                if (info == NULL) {
                        ...
                        if (info) {
                                ...
                                DEV_LOGD_ERR(dev, "%dx%d only support %s(fps)",
...
{
        LOGD_INFO("df_camera_log: %s,%d  ", __func__,on);
        return 0;
}
...
static int max9286_g_input_status(struct v4l2_subdev *sd, u32 *status)
{
        ...
        *status = max9286_read_reg(max9286_data, 0x49);        
        return 0;
}
...
static int max9286_s_stream(struct v4l2_subdev *sd, int enable)
{
        ...
        LOGD_INFO("df_camera_log: %s  ,enable =%d", __func__, enable);
        DEV_LOGD_INFO(sd->dev, "%s", __func__);
        if (enable) {
...
/*
 * Function: max9286_init_work_fun
 * Description: init 9286 and 96705 reg
 * Input: @work,the structure for work
 * Output: void
 * Author: zhangjinqiang
 * Time: 2021/6/5
 */
static void max9286_init_work_fun(struct work_struct *work)
{
        int retval = 0;
        struct sensor_data *max9286_data = container_of(to_delayed_work(work),
        struct sensor_data, init_camera_work);

        LOGD_INFO("%s",__func__);

        if(NULL == max9286_data) {
                LOGD_INFO("can not get max9286_data!!!");
                return;
        }
        ...
        if(max9286_data->sensor_num == 0) {
                DEV_LOGD_ERR(dev, "cameras are not found");
                return;
        }

        retval = max9286_hardware_init(max9286_data);
        if (retval < 0) {
                DEV_LOGD_INFO(dev, "camera init failed");
...
static void max9286_periodic_check_fun(struct work_struct *work)
{
     int reg =0;
     struct sensor_data *max9286_data =  container_of(to_delayed_work(work),struct sensor_data, periodic_check_work);
     
     reg = max9286_read_reg(max9286_data, MAX9286_REG_0X49);

     if(reg < 0)
     {
         LOGD_ERR("%s,current=%d,last=0x%x",__func__,reg,max9286_data->last_camera_status);
         max9286_data->last_camera_status = 0;

        goto RECHECK;
     }
     
     reg =reg &MAX9286_VIDEO_DETECT_MASK;
     
     if((MAX9286_VIDEO_DETECT_MASK == max9286_data->last_camera_status )&&(MAX9286_VIDEO_DETECT_MASK == reg))
     {
         goto RECHECK;
     }
     else
        if((MAX9286_VIDEO_DETECT_MASK != max9286_data->last_camera_status )&&(MAX9286_VIDEO_DETECT_MASK == reg))
         {
            LOGD_ERR("%s:recovery camera,current=0x%x,last=0x%x",__func__,reg,max9286_data->last_camera_status);

            max9286_data->last_camera_status = reg;
            
            schedule_delayed_work(&max9286_data->init_camera_work, 0);
            goto RECHECK;            
         }
        else
            if((MAX9286_VIDEO_DETECT_MASK == max9286_data->last_camera_status )&&(MAX9286_VIDEO_DETECT_MASK != reg))
            {
                    LOGD_ERR("%s:camera link lost,current=0x%x,last=0x%x",__func__,reg,max9286_data->last_camera_status);
      ...
      RECHECK:
          schedule_delayed_work(&max9286_data->periodic_check_work, msecs_to_jiffies(MAX9286_PERIODIC_CHECK));
}
                ...
                const struct i2c_device_id *id)
{
        ...
        if (IS_ERR(max9286_data->sensor_clk)) {
                ...
                DEV_LOGD_ERR(dev, "clock-frequency missing or invalid");
                return PTR_ERR(max9286_data->sensor_clk);
        }

        INIT_DELAYED_WORK(&max9286_data->init_camera_work, max9286_init_work_fun);
        INIT_DELAYED_WORK(&max9286_data->periodic_check_work, max9286_periodic_check_fun);

        retval = of_property_read_u32(dev->of_node, "mclk",
                &(max9286_data->mclk));
        if (retval) {
                DEV_LOGD_ERR(dev, "mclk missing or invalid");
                return retval;
        }
        ...
        if (retval) {
                DEV_LOGD_ERR(dev, "mclk_source missing or invalid");
                return retval;
        }

        max9286_data->pwr_gpio = of_get_named_gpio(dev->of_node, "pwr-gpios", 0);
        if (!gpio_is_valid(max9286_data->pwr_gpio)) {
                DEV_LOGD_ERR(dev, "no camera pwr pin available");
                //return -ENODEV;
        }
        retval = devm_gpio_request_one(dev, max9286_data->pwr_gpio, GPIOF_OUT_INIT_HIGH,
                                        "camera_pwr");
        //if (retval < 0)
        //      return retval;
        camera_pwr_reset(max9286_data);
        /* request power down pin */
        max9286_data->pwn_gpio = of_get_named_gpio(dev->of_node, "pwn-gpios", 0);
        if (!gpio_is_valid(max9286_data->pwn_gpio)) {
                DEV_LOGD_ERR(dev, "no sensor pwdn pin available");
                return -ENODEV;
        }
        ...
        i2c_set_clientdata(client, max9286_data);
        ...
        if (retval != 0x40) {
                LOGD_ERR("max9286 is not found, chip id reg 0x1e = 0x%x.", retval);
                clk_disable_unprepare(max9286_data->sensor_clk);
                devm_gpio_free(dev, max9286_data->pwn_gpio);
                return -ENODEV;
        }

        /*close the front channel*/
        max9286_write_reg(max9286_data, 0x0A, 0x0F);

        /* Disable CSI Output */
        max9286_write_reg(max9286_data, 0x15, 0x03);
        msleep(5);
        #if 0
        /* request camera power  pin  for camera*/
        max9286_data->pwr_gpio = of_get_named_gpio(dev->of_node, "pwr-gpios", 0);
        if (!gpio_is_valid(max9286_data->pwr_gpio)) {
                DEV_LOGD_ERR(dev, "no camera pwr pin available");
                return -ENODEV;
        ...
        if (retval < 0) {
                DEV_LOGD_ERR(&client->dev,
                                        "--Async register failed, ret=%d", retval);
        ...
        ret= sysfs_create_group(&client->dev.kobj, &max9286_group);
        if (ret) {
                DEV_LOGD_ERR(&client->dev, "sysfs_create_group failed(%d)", ret);
        }
        ...
        if (retval < 0) {
                DEV_LOGD_ERR(&client->dev, "create device file fail");
                return retval;
        }
        #endif

        DEV_LOGD_INFO(&max9286_data->i2c_client->dev,
                        "max9286_mipi is found, name %s", sd->name);
        return retval;
}
...
/*
 * Function: max9286_shutdown
 * Description: shutdown max9286 power and pdb
 * Input: @client,the max9286 i2c_client
 * Output: 0, success
 * Author: zhangjinqiang
 * Time: 2021/6/5
 */
static int max9286_shutdown(struct i2c_client *client)
{
        struct v4l2_subdev *sd = i2c_get_clientdata(client);
        struct sensor_data *max9286_data = subdev_to_sensor_data(sd);
        if(gpio_is_valid(max9286_data->pwr_gpio)  && gpio_is_valid(max9286_data->pwn_gpio))
        {
                gpio_set_value(max9286_data->pwr_gpio, 0);
                gpio_set_value(max9286_data->pwn_gpio, 0);
        } else {
                LOGD_INFO("THE 9286 power and pdb error!!!");
        }
        return 0;
}
